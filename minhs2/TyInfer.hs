...
unify (TypeVar v) t
    | (elem v (tv t)) = error("there is no unifier")
...
unify x y = error ("there is no unifier")

generalise :: Gamma -> Type -> QType
generalise g t = foldl Forall (Ty t) ((tv t) \\ (tvGamma g))
...
inferExp g (Con c) = do
    case constType c of
        ...
        Nothing   -> error("Not find such constructor")
...
--Case
inferExp g (Case e [Alt idx [x] e1, Alt idy [y] e2]) = do
        (e', tau, tee)     <- inferExp g e
        alphaL             <- fresh
        ...
        return ((Case e' [Alt idx [x] e1', Alt idy [y] e2']), substitute (u' <> u) tauR, u' <> u <> tee2 <> tee1 <> tee)
...
inferExp g (Recfun (Bind f recType [x] e)) = do
        ...
        return (Recfun (Bind f (Just (Ty (substitute u (Arrow (substitute tee alpha1) tau)))) [x] e', (substitute u (Arrow (substitute tee alpha1) tau)), u <> tee))
